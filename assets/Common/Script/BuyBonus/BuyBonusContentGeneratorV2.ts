...
@ccclass('BuyBonusContentGeneratorV2')
export class BuyBonusContentGeneratorV2 extends Component {
  @property(Node)
  ...
  @property({
    type: TTFFont,
    visible: function (this: BuyBonusContentGeneratorV2) {
  ...
  private generateContent() {
    ...
    trans.height = this.contentHeight;
  ...
  async start() {
    ...
    if (!this.translation) {
      console.warn('[BuyBonusContentGenerator] JSON is null, fallback check', {
        folderPath: this.folderPath,
        lang: PlatformData.lang,
        fileName: this.fileName,
      });
      //load default en-us file
      this.translation = await this.loadJsonFile(
        `${this.folderPath}/en-us/${this.fileName}`
      );
    }
