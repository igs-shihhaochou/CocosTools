...
@ccclass('SpecialGameSample')
export class SpecialGameSample extends SpecialGameBase {
  ...
  //    ///停輪的流程註冊用的function，再這個範例中沒有使用到(參考鳳凰傳說同名的方法)
  //    ///所有輪停止
  private AllWheelStopped(_ctrlIndex: number) {
    // this.scheduleOnce(this.AllWheelStoppedProcess, 0.5);    ////節奏優化
  }
  //    //停輪後的表演流程
  private AllWheelStoppedProcess() {}
  //    ///(必須要Override)
  ...
  private onKeyDown(_event: Event.EventKeyboard) {
    // if ((event as any).keyCode == cc.macro.KEY.space && this.canPressSpaceflag) {
 ...
 */
// import { SpecialGameState, WinType } from "../../Define/SlotGameData";
...
//     ///停輪的流程註冊用的function，再這個範例中沒有使用到(參考鳳凰傳說同名的方法)
//     ///所有輪停止
//     private AllWheelStopped(ctrlIndex: number) {
//
//         this.scheduleOnce(this.AllWheelStoppedProcess, 0.5);    ////節奏優化
//     }
//
