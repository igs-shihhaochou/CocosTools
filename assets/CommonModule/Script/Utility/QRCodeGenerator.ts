/**
 * 純 TypeScript QR Code 產生器
 * 基於 qrcodejs (https://github.com/davidshimjs/qrcodejs) 核心邏輯
 * 支援 Byte mode 編碼、L/M/Q/H 錯誤修正、Version 1~40、輸出 HTMLCanvasElement
 *
...
const QRMathEXP = new Array<number>(256);
const QRMathLOG = new Array<number>(256);

(function initMathTables() {
  for (let i = 0; i < 8; i++) {
    QRMathEXP[i] = 1 << i;
  }
  for (let i = 8; i < 256; i++) {
    QRMathEXP[i] =
      QRMathEXP[i - 4] ^ QRMathEXP[i - 5] ^ QRMathEXP[i - 6] ^ QRMathEXP[i - 8];
  }
  for (let i = 0; i < 255; i++) {
    QRMathLOG[QRMathEXP[i]] = i;
  }
})();
...
class QRPolynomial {
  ...
  multiply(e: QRPolynomial): QRPolynomial {
    const num = new Array<number>(this.getLength() + e.getLength() - 1);
...
function getRSBlocks(typeNumber: number, errorCorrectLevel: number): RSBlock[] {
...
class QR8bitByte {
  ...
  constructor(data: string) {
    ...
    for (let i = 0, l = data.length; i < l; i++) {
      ...
      parsed.push(byteArray);
    }
    this.parsedData = ([] as number[]).concat(...parsed);
...
class QRCodeModel {
  ...
  private getBestMaskPattern(): number {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const lp = getLostPoint(this.modules!);
...
function getTypeNumber(sText: string, errorCorrectLevel: number): number {
  let nType = 1;
...
const EC_LEVEL_VALUE: Record<ECLevel, number> = {
...
export class QRCodeGenerator {
  /**
   * 產生 QR Code 並回傳 HTMLCanvasElement
   * @param text 要編碼的文字
   * @param options 選項
   */
  static generate(text: string, options?: QRCodeOptions): HTMLCanvasElement {
    const ecLevel = options?.errorCorrectionLevel ?? 'M';
    const cellSize = options?.cellSize ?? 4;
    const margin = options?.margin ?? 2;
    const ecValue = EC_LEVEL_VALUE[ecLevel];

    const typeNumber = getTypeNumber(text, ecValue);
    const model = new QRCodeModel(typeNumber, ecValue);
    model.addData(text);
    model.make();
