...
export class UISuperScrollView extends ScrollView {
  private direction: ScrollViewDirection = ScrollViewDirection.NONE;
  private _layout!: UISuperLayout;
  @property({
    tooltip:
      '注意！向上传递事件只会发送当前滑动相反方向,如果开启horizontal则会发送vertical事件。如果开启vertical则会发送horizontal事件。同时开启horizontal和vertical 不会发送任何事件',
  })
  isTransmitEvent = false;
  @property pullRefresh = false;
  @property({
    displayName: '顶部偏移量',
    tooltip: '下拉时超过此偏移会发送下拉事件',
    visible: function () {
      return (this as any).pullRefresh;
    },
  })
  headerOutOffset = 200;
  @property({
  ...
  public onEnable() {
    super.onEnable();
    this.node.on(
      PageView.EventType.SCROLL_ENG_WITH_THRESHOLD,
      this.dispatchPageTurningEvent,
      this
    );
  }
  ...
  private isCallSoonFinish = false;
  get curPageIdx() {
  ...
  protected _getContentTopBoundary() {
    ...
    const offset =
      this.layout.isOfTopBoundary === 0
        ? this._topBoundary
        : this.layout.isOfTopBoundary;
    return offset;
  }
  protected _getContentBottomBoundary() {
    ...
    const offset =
      this.layout.isOfButtomBoundary === 0
        ? this._bottomBoundary
        : this.layout.isOfButtomBoundary;
    return offset;
  }
  protected _getContentLeftBoundary() {
    ...
    const offset =
      this.layout.isOfLeftBoundary === 0
        ? this._leftBoundary
        : this.layout.isOfLeftBoundary;
    return offset;
  }
  protected _getContentRightBoundary() {
    ...
    const offset =
      this.layout.isOfRightBoundary === 0
        ? this._rightBoundary
  ...
  protected _onTouchMoved(event: EventTouch, captureListeners: any) {
    if (this.isTransmitEvent) {
      if (this.direction === ScrollViewDirection.NONE) {
        ...
        const curre = event.getLocation();
        const xOffset = Math.abs(start.x - curre.x);
        const yOffset = Math.abs(start.y - curre.y);
        if (xOffset > yOffset) {
      ...
      const canTransmit =
        (this.vertical && this.direction === ScrollViewDirection.HORIZONTAL) ||
    ...
    if (this.pullRefresh) {
      ...
      if (offset > 0 && !this.isLockHeader && !this.isLockFooter) {
        this.headerProgress =
          offset < EPSILON ? 0 : offset / this.headerOutOffset;
        ...
        this.footerProgress =
          -offset < EPSILON ? 0 : -offset / this.footerOutOffset;
  ...
  protected _processAutoScrolling(dt: number) {
    ...
    const fireEvent =
      Math.abs(percentage - 1) <= this.getScrollEndedEventTiming();
    ...
    if (
      ...
      Math.abs(deltaMove.y) <= 2 &&
      !this.isCallSoonFinish
    ) {
      this.layout['soonFinish']();
      this.isCallSoonFinish = true;
    } else if (
      !this.layout.vertical &&
      Math.abs(deltaMove.x) <= 2 &&
      !this.isCallSoonFinish
    ) {
      this.layout['soonFinish']();
      this.isCallSoonFinish = true;
  ...
  release() {
    ...
    if (this.isLockHeader || this.isLockFooter) {
      if (this.vertical && this.isLockHeader) {
        this._topBoundary += this.headerOutOffset;
      }
      if (this.vertical && this.isLockFooter) {
        this._bottomBoundary -= this.footerOutOffset;
      }
      if (this.horizontal && this.isLockHeader) {
        this._leftBoundary -= this.headerOutOffset;
      }
      if (this.horizontal && this.isLockFooter) {
  ...
  protected _updateScrollBar(outOfBoundary: any) {
    ...
    if (offset > 0) {
      // 下滑
      const progress = offset < EPSILON ? 0 : offset / this.headerOutOffset; //根据参数 headerOutOffset 计算当前下滑的办百分比
      if (this.isLockHeader) {
        ...
        this.headerProgress =
          progress < this.headerProgress ? progress : this.headerProgress;
        EventHandler.emitEvents(this.headerEvents, this, {
          action: false,
          progress: this.headerProgress,
          stage: 'release',
      ...
      const progress = -offset < EPSILON ? 0 : -offset / this.footerOutOffset; //根据参数 footerOutOffset 计算当前下滑的办百分比
      if (this.isLockFooter) {
        this.footerProgress =
          this.footerProgress === 1
            ? this.footerProgress
            : Math.max(progress, 1);
        EventHandler.emitEvents(this.footerEvents, this, {
          action: false,
        ...
        this.footerProgress =
          progress < this.footerProgress ? progress : this.footerProgress;
        EventHandler.emitEvents(this.footerEvents, this, {
          action: false,
          progress: this.footerProgress,
          stage: 'release',
  ...
  protected _autoScrollToPage() {
    ...
    if (bounceBackStarted) {
      ...
      if (bounceBackAmount.x > 0 || bounceBackAmount.y < 0) {
        if (this.layout.horizontal) {
          if (
            ...
            UISuperLayout.HorizontalAxisDirection.LEFT_TO_RIGHT
          ) {
            this.layout['_currPageIndex'] =
              this.layout.itemTotal === 0 ? 0 : this.layout.itemTotal - 1;
          } else {
            this.layout['_currPageIndex'] = 0;
          }
        } else {
          if (
            this.layout.verticalAxisDirection ===
            UISuperLayout.VerticalAxisDirection.TOP_TO_BOTTOM
          ) {
            this.layout['_currPageIndex'] =
              this.layout.itemTotal === 0 ? 0 : this.layout.itemTotal - 1;
          } else {
      ...
      const timeInSecond =
        this.layout.pageTurningSpeed * Math.abs(index - nextIndex);
  ...
  protected isQuicklyScrollable(touchMoveVelocity: Vec3) {
    if (this.horizontal) {
      if (
        Math.abs(touchMoveVelocity.x) > this.layout.autoPageTurningThreshold
      ) {
        return true;
      }
    } else if (this.vertical) {
      if (
        Math.abs(touchMoveVelocity.y) > this.layout.autoPageTurningThreshold
      ) {
        return true;
      }
    }
  ...
  // 是否超过自动滚动临界值
  protected isScrollable(offset: Vec2, _index: number, _nextIndex: number) {
    ...
    if (this.horizontal) {
      return (
        Math.abs(offset.x) >= viewTrans.width * this.layout.scrollThreshold
      );
    } else if (this.vertical) {
      return (
        Math.abs(offset.y) >= viewTrans.height * this.layout.scrollThreshold
      );
    }
    return false;
  }
  protected transmitEvent(event: EventTouch, eventType: string) {
    const e = new EventTouch(event.getTouches(), event.bubbles, eventType);
    e.touch = event.touch;
