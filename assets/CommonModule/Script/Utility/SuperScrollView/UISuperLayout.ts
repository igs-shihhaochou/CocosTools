...
export default class UISuperLayout extends Component {
  static VerticalAxisDirection = VerticalAxisDirection;
  static HorizontalAxisDirection = HorizontalAxisDirection;

  @property(UISuperScrollView) scrollView!: UISuperScrollView;
  ...
  @property({
    ...
    visible: function () {
      return (
        (this as any).layoutType === Type.VERTICAL && !(this as any).autoCenter
      );
    },
  })
  ...
  @property({
    ...
    visible: function () {
      return (
        (this as any).layoutType === Type.HORIZONTAL &&
        !(this as any).autoCenter
      );
    },
  })
  indexHorizontalAxisDirection = IndexHorizontalAxisDirection.LEFT;
  @property({type: VerticalAxisDirection}) verticalAxisDirection =
    VerticalAxisDirection.TOP_TO_BOTTOM;
  @property({type: HorizontalAxisDirection}) horizontalAxisDirection =
    HorizontalAxisDirection.LEFT_TO_RIGHT;

  @property({tooltip: '最小值=1，大于1就是Grid模式'}) groupItemTotal = 1;
  @property({tooltip: '决定最多创建Prefab的数量'}) multiple = 2;
  @property({tooltip: '顶部填充'}) paddingTop = 0;
  @property({tooltip: '底部填充'}) paddingBottom = 0;
  @property({tooltip: '左侧填充'}) paddingLeft = 0;
  @property({tooltip: '右侧填充'}) paddingRight = 0;
  @property({tooltip: '横轴间距'}) spacingX = 0;
  @property({tooltip: '纵轴间距'}) spacingY = 0;
  @property({tooltip: '计算缩放后的尺寸'}) affectedByScale = false;

  @property({tooltip: '开启翻页模式'}) isPageView = false;
  @property({
    tooltip: '每个页面翻页时所需时间。单位：秒',
    visible: function () {
      return (this as any).isPageView;
    },
  })
  ...
  centerAnchor: Vec2 = new Vec2(0.5, 0.5);

  @property({tooltip: '上/左 无限循环'}) headerLoop = false;
  @property({tooltip: '下/右 无限循环'}) footerLoop = false;
  ...
  get transform(): UITransform | null {
    return (
  ...
  get footer(): UITransform | null {
    if (this.node.children.length === 0) return null;
    return this.node.children[this.node.children.length - 1].getComponent(
      UITransform
    );
  }
  /** 头部索引 */
  get headerIndex(): number {
    ...
    const node: any = this.header.node;
    return node['__index'];
  }
  /** 底部索引 */
  get footerIndex(): number {
    if (!this.footer) return -1;
    const node: any = this.footer.node;
    return node['__index'];
  }
  /** Item起始位置 */
  ...
  get headerBoundary(): number {
    ...
    if (this.vertical) {
      if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
        return (
          this.node.position.y + this.getItemYMax(this.header) + this.paddingTop
        );
      } else {
        return (
          this.node.position.y +
          this.getItemYMin(this.header) -
          this.paddingBottom
        );
      }
    } else {
      if (
        ...
        return (
          ...
          this.getItemXMin(this.header) -
          this.paddingLeft
        );
      } else {
        return (
          this.node.position.x +
          this.getItemXMax(this.header) +
          this.paddingRight
        );
      }
    }
  ...
  get footerBoundary(): number {
    ...
    if (this.vertical) {
      if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
        return (
          this.node.position.y +
          this.getItemYMin(this.footer) -
          this.paddingBottom
        );
      } else {
        return (
          this.node.position.y + this.getItemYMax(this.footer) + this.paddingTop
        );
      }
    } else {
      if (
        ...
        return (
          this.node.position.x +
          this.getItemXMax(this.footer) +
          this.paddingRight
        );
      } else {
        return (
          this.node.position.x +
          this.getItemXMin(this.footer) -
          this.paddingLeft
        );
      }
    }
    ...
    if (this.centerNode) {
      if (this.vertical) {
        if (
          ...
          return (
            this.headerBoundary +
            (this.viewHeaderBoundary - (this.centerNode.position as any)[key])
          );
        } else {
          return (
            this.footerBoundary +
      ...
      if (this.vertical) {
        if (
          ...
          return (
            this.headerBoundary +
            (this.viewHeaderBoundary - (this.view.node.position as any)[key])
          );
        } else {
          return (
            this.footerBoundary +
    ...
    if (this.centerNode) {
      if (this.vertical) {
        if (
          ...
          return (
            this.footerBoundary +
            (this.viewFooterBoundary - (this.centerNode.position as any)[key])
          );
        } else {
          return (
            this.headerBoundary +
      ...
      if (this.vertical) {
        if (
          ...
          return (
            this.footerBoundary +
            (this.viewFooterBoundary - (this.view.node.position as any)[key])
          );
        } else {
          return (
            this.headerBoundary +
  ...
  get fixedItemHeight(): number {
    ...
    if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
      return (
        Math.abs(this.getItemYMax(this.header)) +
        Math.abs(this.getItemYMin(this.footer))
      );
    } else {
      return (
        Math.abs(this.getItemYMin(this.header)) +
        Math.abs(this.getItemYMax(this.footer))
      );
    }
  }
  /** 从头部到底部的所有Item宽度总和 */
  get fixedItemWidth(): number {
    ...
    if (
      ...
      return (
        Math.abs(this.getItemXMin(this.header)) +
        Math.abs(this.getItemXMax(this.footer))
      );
    } else {
      return (
        Math.abs(this.getItemXMax(this.header)) +
        Math.abs(this.getItemXMin(this.footer))
      );
    }
  }
  ...
  get contentSize(): Size {
    ...
    const size = new Size(
      this.view.contentSize.width,
      this.view.contentSize.height
    );
    if (this.vertical) {
      if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
  ...
  private _maxPrefabTotal = 0;
  /** 已被创建的Item数量 */
  get maxPrefabTotal(): number {
    return this._maxPrefabTotal;
  }
  private currentCreateItemTotal = 0;
  ...
  get centerPosition(): Vec3 {
    if (!this._centerPosition) {
      ...
      if (this.autoCenter) {
        if (this.centerNode) {
          const worldPos = this.centerNode.parent
            ?.getComponent(UITransform)
            ?.convertToWorldSpaceAR(this.centerNode.position);
          this._centerPosition = this.view.convertToNodeSpaceAR(worldPos);
        }
      } else {
        if (this.vertical) {
          if (
            this.indexVerticalAxisDirection === IndexVerticalAxisDirection.TOP
          ) {
          ...
          if (
            this.indexHorizontalAxisDirection ===
            IndexHorizontalAxisDirection.LEFT
          ) {
  ...
  onLoad() {
    ...
    this.scrollView.view?.node.on(
      Node.EventType.SIZE_CHANGED,
      this.onViewSizeChange,
      this
    );
    ...
    Object.defineProperty(this.transform, 'width', {
      ...
      set: (width: number) => {
        this.transform?.setContentSize(
          new Size(width, this.contentSize.height)
        );
      },
    });
    Object.defineProperty(this.transform, 'height', {
      ...
      set: (height: number) => {
        this.transform?.setContentSize(
          new Size(this.contentSize.width, height)
        );
      },
    });
  }
  ...
  scrollToHeader(timeInSecond?: number) {
    ...
    this.scrollToIndex(
      0,
      timeInSecond,
      new Vec3(headerOrFooter, headerOrFooter)
    );
  }
  /** 滚动到尾部 */
  ...
  private isNearFooter(index: number) {
    ...
    const flag = index > this.footerIndex && index < this.headerIndex;
    if (flag) {
      const result =
        Math.abs(index - this.headerIndex) < Math.abs(index - this.footerIndex);
      if (this.vertical) {
        if (
          this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM
        ) {
  ...
  private getFooterOffset(index: number) {
    const footerOffset = this.footerIndex % this.groupItemTotal;
    const indexOffset = index % this.groupItemTotal;
    return indexOffset - footerOffset + this.groupItemTotal;
  }
  private getHeaderOffset(index: number) {
    const headerOffset = this.headerIndex % this.groupItemTotal;
    const indexOffset = index % this.groupItemTotal;
  ...
  scrollToIndex(
    ...
    let child = this.node.children.find(
      (item: any) => item['__index'] === index
    );
    let nearFooter = false;
    if (!child) {
      ...
      const flag =
        (this.vertical &&
          this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) ||
        (!this.vertical &&
          this.horizontalAxisDirection ===
            HorizontalAxisDirection.LEFT_TO_RIGHT);
      if (nearFooter) {
        this.offsetToFooter(index);
        if (flag) {
          this.resetIndexEndToStart(index);
        } else {
          this.resetIndexStartToEnd(index);
        }
      } else {
    ...
    if (!this.autoCenter) {
      if (this.vertical) {
        if (
          ...
          if (!reverse) {
            itemPos.y =
              this.getItemYMax(child._uiProps.uiTransformComp!) +
              this.paddingTop;
          } else {
            itemPos.y =
              this.getItemYMin(child._uiProps.uiTransformComp!) -
              this.paddingBottom;
          }
        } else {
          if (!reverse) {
        ...
        if (
          ...
          if (!reverse) {
            itemPos.x =
              this.getItemXMin(child._uiProps.uiTransformComp!) -
              this.paddingLeft;
          } else {
            itemPos.x =
              this.getItemXMax(child._uiProps.uiTransformComp!) +
              this.paddingRight;
          }
        } else {
          if (!reverse) {
    ...
    if (!this.autoCenter && boundary) {
      ...
      multiple = this.getCenterAnchor(
        child._uiProps.uiTransformComp!,
        this.centerPosition
      );
    }
  ...
  protected soonFinish() {
    ...
    this.scrollView.stopAutoScroll();
    let findedPos = new Vec3(999999, 999999);
    for (let i = 0; i < this.node.children.length; i++) {
      const child = this.node.children[i];
      const worldPos = this.transform?.convertToWorldSpaceAR(child.position);
      const localPos = this.view.convertToNodeSpaceAR(worldPos);
      const map = {width: false, height: false};

      const multiple = this.getCenterAnchor(
        child._uiProps.uiTransformComp!,
        this.centerPosition
      );
  ...
  /** 计算垂直模式的Item应该的位置 */
  protected getVerticalRelativePosition(
    item: UITransform,
    relative: UITransform,
    reverse: boolean
  ) {
    const pos = new Vec3();
    let isAccommodate = false;
    if (
      this.horizontalAxisDirection === HorizontalAxisDirection.LEFT_TO_RIGHT
    ) {
      isAccommodate = !reverse
        ? this.isAccommodateByRight(relative)
        : this.isAccommodateByLeft(relative);
    } else {
      isAccommodate = !reverse
        ? this.isAccommodateByLeft(relative)
        : this.isAccommodateByRight(relative);
    ...
    if (
      ...
      if (!reverse) {
        pos.x = isAccommodate
          ? this.getRelativeByRight(item, relative)
          : this.getStartX(item);
      } else {
        pos.x = isAccommodate
          ? this.getRelativeByLeft(item, relative)
          : this.getEndX(item);
      }
    } else {
      if (!reverse) {
    ...
    if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
      if (!reverse) {
        pos.y = isAccommodate
          ? relative.node.position.y
          : this.getRelativeByBottom(item, relative);
      } else {
        pos.y = isAccommodate
          ? relative.node.position.y
          : this.getRelativeByTop(item, relative);
      }
    } else {
      if (!reverse) {
        pos.y = isAccommodate
  ...
  /** 计算水平模式的Item应该的位置 */
  protected getHorizontalRelativePosition(
    item: UITransform,
    relative: UITransform,
    reverse: boolean
  ) {
    ...
    if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
      isAccommodate = !reverse
        ? this.isAccommodateByBottom(relative)
        : this.isAccommodateByTop(relative);
    } else {
      isAccommodate = !reverse
        ? this.isAccommodateByTop(relative)
        : this.isAccommodateByBottom(relative);
  ...
  /** 创建Item */
  protected async createItems(count: number, force = false) {
    this.gener?.return('');
    if (force) {
    ...
    // 开始分帧创建item
    const total = count - this.node.children.length; //计算当前应该创建的总数
    this.gener = this.getGeneratorLength(total, () => {
      const child: any = instantiate(this.prefab);
  ...
  /** 重新计算当前所有Item的位置 */
  protected resetChilds(start = false) {
    let prevItem: any = this.header;
    if (
      (this.vertical && this.fixedItemHeight <= this.view.height) ||
      (!this.vertical && this.fixedItemWidth <= this.view.width)
    ) {
      const x = this.getStartX(this.header);
      const y = this.getStartY(this.header);
    ...
    for (let i = 1; i < this.node.children.length; i++) {
      const child = this.node.children[i];
      const transform = child._uiProps.uiTransformComp!;
      const relative = !prevItem ? this.header : prevItem;
  ...
  protected pushToFooter(force = false) {
    if (this.vertical) {
      if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
        if (
          force ||
          this.headerBoundary - this.paddingTop >
            this.viewHeaderBoundary + this.header?.height
        ) {
        ...
        if (
          force ||
          this.footerBoundary - this.paddingTop >
            this.viewHeaderBoundary + this.header?.height
        ) {
      ...
      if (
        ...
        if (
          force ||
          this.headerBoundary + this.paddingLeft <
            this.viewHeaderBoundary - this.header?.width
        ) {
          this.pushToFooterHandler();
        }
      } else {
        if (
          force ||
          this.footerBoundary + this.paddingLeft <
            this.viewHeaderBoundary - this.header?.width
        ) {
          this.pushToHeaderHandler();
        }
  ...
  protected pushToHeader(force = false) {
    if (this.vertical) {
      if (this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM) {
        if (
          force ||
          this.footerBoundary + this.paddingBottom <
            this.viewFooterBoundary - this.footer?.height
        ) {
          this.pushToHeaderHandler();
        }
      } else {
        if (
          force ||
          this.headerBoundary + this.paddingBottom <
            this.viewFooterBoundary - this.footer?.height
        ) {
          this.pushToFooterHandler();
        }
      ...
      if (
        ...
        if (
          force ||
          this.footerBoundary - this.paddingRight >
            this.viewFooterBoundary + this.footer?.width
        ) {
          this.pushToHeaderHandler();
        }
      } else {
        if (
          force ||
          this.headerBoundary - this.paddingRight >
            this.viewFooterBoundary + this.footer?.width
        ) {
          this.pushToFooterHandler();
        }
  ...
  protected pushToHeaderHandler() {
    ...
    if (!loop && this.headerIndex === 0) {
      ...
      if (this.vertical) {
        accommodate =
          this.horizontalAxisDirection === HorizontalAxisDirection.LEFT_TO_RIGHT
            ? this.isAccommodateByLeft(this.header!)
            : this.isAccommodateByRight(this.header!);
      } else {
        accommodate =
          this.verticalAxisDirection === VerticalAxisDirection.TOP_TO_BOTTOM
            ? this.isAccommodateByTop(this.header!)
            : this.isAccommodateByBottom(this.header!);
      }
      if (accommodate) {
        this.resetChilds(true);
      }
  ...
  protected notifyRefreshItem(target: Node) {
    EventHandler.emitEvents(
      this.refreshItemEvents,
      target,
      (target as any)['__index']
    );
  }
  /** 分帧加载 */
  protected *getGeneratorLength(
    length: number,
    callback: Function,
    ...params: any
  ): Generator {
    for (let i = 0; i < length; i++) {
      const result = callback(i, ...params);
      if (result) {
        yield;
      } else {
        return;
