...
export default class TmpRichText extends Component {
  ...
  private _updateRichTextPosition(): void {
    ...
    for (let i = 0; i < this._segments.length; ++i) {
      ...
      if (lineCount === nextLineIndex) {
        ...
        const tmp: TextMeshPro = segment.node.getComponent(TextMeshPro);
  ...
  private _convertLiteralColorValue(color: string): Color {
    ...
    if (Color[colorValue]) {
      ...
      const hexString = color.indexOf('#') === 0 ? color.substring(1) : color;
      const r = parseInt(hexString.substring(0, 2), 16) || 0;
      const g = parseInt(hexString.substring(2, 4), 16) || 0;
      const b = parseInt(hexString.substring(4, 6), 16) || 0;
  ...
  private _applyTextAttribute(labelSeg: ISegment): void {
    const labelComponent: TextMeshPro = labelSeg.node.getComponent(TextMeshPro);
    if (!labelComponent) {
      return;
    }
    ...
    labelComponent.colorGradient = Boolean(
    ...
    if (textStyle && textStyle.face) {
      labelComponent.tmpUniform.faceColor = this._convertLiteralColorValue(
        textStyle.face.color
      );
    ...
    if (textStyle && textStyle.outline) {
      labelComponent.tmpUniform.outlineColor = this._convertLiteralColorValue(
        textStyle.outline.color
      );
    ...
    if (labelComponent.tmpUniform.enableUnderlay) {
      labelComponent.tmpUniform.underlayColor = this._convertLiteralColorValue(
        textStyle.underlay.color
      );
    ...
    if (labelComponent.tmpUniform.enableGlow) {
      labelComponent.tmpUniform.glowColor = this._convertLiteralColorValue(
        textStyle.glow.color
      );
