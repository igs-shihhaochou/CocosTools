...
export class TmpUniform {
  ...
  public init(text: TextMeshPro) {
    ...
    const material = this._comp.getMaterialInstance(0);
...
export default class TextMeshPro extends UIRenderer {
  ...
  @property
  private _enableUnderline = false;
  ...
  @property
  private _enableStrikethrough = false;
  ...
  @property
  private _strikethroughOffset = 0;
  @property({
    tooltip: '删除线高度偏移',
    visible() {
      return this._enableStrikethrough;
  ...
  @property
  private _colorGradient = false;
  @property({tooltip: '是否启用颜色渐变，会和顶点颜色混合为最终的顶点颜色'})
  public get colorGradient(): boolean {
    return this._colorGradient;
  }
  public set colorGradient(v: boolean) {
    if (!EDITOR && this._colorGradient === v) {
      return;
    }
  ...
  private _colorExtraDirty = false;

  private _richTextDeltaX = 0;
  ...
  private editorInit(): void {
    if (EDITOR) {
      ...
      // log(this._font);
      //@ts-ignore
      Editor.Message.request('asset-db', 'query-url', this._font['_uuid']).then(
        (url: string) => {
          // log(url);
          if (!url) {
            return;
          ...
          const dir = url.slice(start, end + 1);
          const arr: Promise<Texture2D>[] = [];
          this._font.json['pageData'].forEach(v => {
            const imgUrl = dir + v.file + '/texture';
            arr.push(TmpUtils.load<Texture2D>(imgUrl));
          });
          Promise.all(arr).then(v => {
            this.textures = v;
            this._fontConfig = new TmpFontConfig(
              this._font.json,
              this.textures
            );
  ...
  private commitComp(
    ...
    if (render._currMaterial !== mat) {
      ...
      if (isMatEqual) {
        ...
        const renderMat = comp.getRenderMaterial(0);
        for (let i = 0; i < arr.length; i++) {
          const propName = arr[i];
          const v1 = renderMat.getProperty(propName);
          const v2 = render._currMaterial.getProperty(propName);
