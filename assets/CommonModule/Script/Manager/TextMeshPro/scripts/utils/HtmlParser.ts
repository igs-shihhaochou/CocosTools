...
export class HtmlTextParser {
  ...
  private _attributeToObject(attribute: string) {
    ...
    if (header && header[0].length > 0) {
      ...
      if (tagName.startsWith('br') && tagName[tagName.length - 1] === '/') {
        ...
        this._resultObjectArray.push({
          text: '',
          style: {isNewLine: true} as IHtmlTextParserStack,
        });
        return obj;
      }
    }
      ...
      if (tagName.startsWith('img') && tagName[tagName.length - 1] === '/') {
        ...
        while (header) {
          ...
          nextSpace = remainingArgument.indexOf(
            ' ',
            rightQuot + 1 >= remainingArgument.length ? -1 : rightQuot + 1
          );
          tagValue =
            nextSpace > -1
    ...
    if (header) {
      ...
      const defaultColorGradientObject = {
        lb: '#ffffff',
        rb: '#ffffff',
        lt: '#ffffff',
        rt: '#ffffff',
      ...
      if (attribute) {
        const colorGradientAttrReg =
          /(\s)*lb(\s)*=|(\s)*rb(\s)*=|(\s)*lt(\s)*=|(\s)*rt(\s)*=/;
    ...
    if (header) {
      ...
      const defaultFaceObject = {color: '#ffffff', dilate: 0.5, softness: 0.01};
      if (attribute) {
        const faceAttrReg =
          /(\s)*color(\s)*=|(\s)*dilate(\s)*=|(\s)*softness(\s)*=|(\s)*click(\s)*=|(\s)*param(\s)*=/;
    ...
    if (header) {
      ...
      if (attribute) {
        const outlineAttrReg =
          /(\s)*color(\s)*=|(\s)*thickness(\s)*=|(\s)*click(\s)*=|(\s)*param(\s)*=/;
    ...
    if (header) {
      ...
      if (attribute) {
        const underlayAttrReg =
          /(\s)*color(\s)*=|(\s)*x(\s)*=|(\s)*y(\s)*=|(\s)*dilate(\s)*=|(\s)*softness(\s)*=|(\s)*click(\s)*=|(\s)*param(\s)*=/;
    ...
    if (header) {
      ...
      if (attribute) {
        const glowAttrReg =
          /(\s)*color(\s)*=|(\s)*offset(\s)*=|(\s)*inner(\s)*=|(\s)*outer(\s)*=|(\s)*power(\s)*=|(\s)*click(\s)*=|(\s)*param(\s)*=/;
  ...
  private getRightQuotationIndex(remainingArgument: string) {
    ...
    const useSingleQuot =
      leftSingleQuot > -1 &&
      (leftSingleQuot < leftDoubleQuot || leftDoubleQuot === -1);
    const useDoubleQuot =
      leftDoubleQuot > -1 &&
      (leftDoubleQuot < leftSingleQuot || leftSingleQuot === -1);
    if (useSingleQuot) {
      leftQuot = leftSingleQuot;
      rightQuot = remainingArgument.indexOf(
        "'",
        leftQuot + 1 >= remainingArgument.length ? -1 : leftQuot + 1
      );
