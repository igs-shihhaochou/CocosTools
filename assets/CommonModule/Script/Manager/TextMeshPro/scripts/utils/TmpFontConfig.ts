...
export default class TmpFontConfig {
  ...
  public constructor(json: any, textures: Texture2D[]) {
    ...
    json.charData.forEach(v => {
      const letter = new TmpFontLetter();
  ...
  public addLetterDefinitions(
    letter: number,
    letterDefinition: TmpFontLetter
  ): void {
  ...
  public cloneLetterDefinition(): {[id: number]: TmpFontLetter} {
    ...
    for (const key in this._letterDefinitions) {
      const value = new TmpFontLetter();
  ...
  public getLetterDefinitionForChar(char: string): TmpFontLetter {
    ...
    const hasKey = this._letterDefinitions.hasOwnProperty(key);
    let letter;
    if (hasKey) {
