...
const _tmpUvRect = rect();
const _tmpPosRect = rect();
const _horizontalKernings = [];
const _linesWidth = [];
const _linesOffsetX = [];
...
export default class TmpAssembler {
  /** 每个顶点的数据长度 */
  protected static floatsPerVert = 14;
  protected static verticesCount = 4;
  protected static indicesCount = 6;
  protected static uvOffset = 3;
  protected static colorOffset = 5;
  protected static colorExtraOffset = 9;
  protected static textureIdxOffset = 13;

  static createData(comp: TextMeshPro) {
    return comp.requestRenderData();
  }

  static fillBuffers(comp: TextMeshPro, _renderer: any) {
  ...
  private static _updateProperties(comp: TextMeshPro): void {
    ...
    if (comp.overflow === TmpOverflow.ELLIPSIS) {
      ...
      if (_ellipsisDef) {
        _ellipsisWidth =
          (_ellipsisDef.xAdvance * _bmfontScale + _spacingX) * ELLIPSIS_NUM;
      } else {
        _ellipsisWidth = 0;
        log(
  ...
  private static _computeHorizontalKerningForText(): void {
    ...
    const _stringLen = string.length;
  ...
  private static _multilineTextWrap(nextTokenFunc: Function): boolean {
    ...
    for (let index = 0; index < textLen; ) {
      ...
      if (character === '\n') {
        ...
        if (
          ...
          useEllipsis = true;
          // 更新_linesWidth
          const ellipsisInfo = _comp.lettersInfo[_comp.lettersInfo.length - 1];
          // letterRight = ellipsisInfo.x + (_ellipsisDef.w) * _bmfontScale - shareLabelInfo.margin;
          letterRight =
            ellipsisInfo.x +
            (_ellipsisDef.xAdvance - _ellipsisDef.offsetX) * _bmfontScale +
            _spacingX -
      ...
      for (let tmp = 0; tmp < tokenLen; ++tmp) {
        ...
        if (_overflow === TmpOverflow.ELLIPSIS && _ellipsisDef) {
          if (
            letterX + (letterDef.xAdvance - letterDef.offsetX) * _bmfontScale >
            _maxLineWidth
          ) {
        ...
        letterPosition.y =
          nextTokenY - letterDef.offsetY * _bmfontScale + shareLabelInfo.margin;
    ...
    if (_labelHeight <= 0) {
      _contentSize.height =
        parseFloat(_textDesiredHeight.toFixed(2)) + shareLabelInfo.margin * 2;
    }
  ...
  private static _recordEllipsis(
    ...
    while (lastIndex >= 0) {
      const lastInfo = _comp.lettersInfo[lastIndex];
      const lastDef = shareLabelInfo.fontAtlas.getLetterDefinitionForChar(
        lastInfo.char
      );
      const lastW = lastDef ? lastDef.w : 0;
      const lastXAdvance = lastDef ? lastDef.xAdvance : 0;
      const lastOffsetX = lastDef ? lastDef.offsetX : 0;
      const lastRightX =
    ...
    // 记录省略号
    letterPosition.y =
      nextTokenY - _ellipsisDef.offsetY * _bmfontScale + shareLabelInfo.margin;
    for (let i = 1; i <= ELLIPSIS_NUM; i++) {
      letterPosition.x =
        nextX + _ellipsisDef.offsetX * _bmfontScale - shareLabelInfo.margin;
      this._recordLetterInfo(
        letterPosition,
        ELLIPSIS_CHAR,
        lastIndex + i,
  ...
  private static _recordPlaceholderInfo(
    ...
    if (letterIndex >= _comp.lettersInfo.length) {
      const tmpInfo = new TmpLetterInfo();
  ...
  private static _updateQuads(): void {
    ...
    for (let i = 0, l = _comp.lettersInfo.length; i < l; ++i) {
      const letterInfo = _comp.lettersInfo[i];
      if (!letterInfo) break;
      ...
      if (_labelWidth > 0) {
        if (_overflow === TmpOverflow.CLAMP) {
          ...
          if (px + _tmpUvRect.width * _bmfontScale > _contentSize.width) {
            const clipRight =
              px + _tmpUvRect.width * _bmfontScale - _contentSize.width;
      ...
      if (_tmpUvRect.height > 0 && _tmpUvRect.width > 0) {
        ...
        if (
          ...
          if (!TmpUtils.isUnicodeSpace(letterInfo.char)) {
            const lineData = _extraLinesData[letterInfo.line];
            if (!lineData) {
    ...
    if (_extraLineDef) {
      ...
      if ((_comp as TextMeshPro).enableUnderline) {
        this._updateLineQuads(
          appX,
          appY,
          -_fontSize + (_comp as TextMeshPro).underlineOffset * _bmfontScale
        );
      }
      // 删除线
      if ((_comp as TextMeshPro).enableStrikethrough) {
        this._updateLineQuads(
          ...
          appY,
          -_fontSize / 2 +
            (_comp as TextMeshPro).strikethroughOffset * _bmfontScale
        );
      }
    }
  ...
  private static _updateLineQuads(
    ...
    for (const key in _extraLinesData) {
      const underlineInfo = _extraLinesData[key];
      const lineIdx = underlineInfo.lineIndex;
      const first = underlineInfo.first;
      const last = underlineInfo.last > 0 ? underlineInfo.last : first;
      const firstInfo = _comp.lettersInfo[first];
      if (!firstInfo.valid) {
        continue;
      }
      const lastInfo = _comp.lettersInfo[last];
      const firstDef = shareLabelInfo.fontAtlas.getLetter(firstInfo.hash);
      const lastDef = shareLabelInfo.fontAtlas.getLetter(lastInfo.hash);

      const maxWidth = lastInfo.x + lastDef.w * _bmfontScale - firstInfo.x;
      ...
      const wLeft =
        maxWidth >= _extraLineDef.w * _bmfontScale
          ...
          : maxWidth / 2;
      const wRight = wLeft;
      const wMid = maxWidth - wLeft - wRight;
      const leftX = firstInfo.x + _linesOffsetX[lineIdx];
      const rightX = leftX + wLeft + wMid;
      const midX = leftX + wLeft;
      ...
      if (_labelHeight > 0) {
        ...
        if (
          ...
          _tmpUvRect.height =
            py < _tailoredBottomY ? 0 : (py - _tailoredBottomY) / _bmfontScale;
        }
      }
  ...
  private static appendQuad(
    ...
    const texture = shareLabelInfo.fontAtlas.getTexture(textureId);
  ...
  public static updateColorExtra(comp: TextMeshPro): void {
    ...
    if (!JSB) {
      for (let i = 0; i < comp.lettersInfo.length; i++) {
        const info = comp.lettersInfo[i];
        if (!info.valid || TmpUtils.isUnicodeSpace(info.char)) {
          continue;
        }
        const alpha = info.visible ? 1 : 0;
        const offset = info.quadsIndex * 4;
      ...
      for (let i = 0; i < quadCount; i++) {
        ...
        if (letterIndex < comp.lettersInfo.length) {
          const info = comp.lettersInfo[letterIndex];
          const alpha = info.visible ? 1 : 0;
          const offset = i * 4;
          if (dataList.length < offset + 4) {
            break;
  ...
  public static isVisble(comp: TextMeshPro, index: number): boolean {
    const info = comp.lettersInfo[index];
    return (
      info && info.valid && info.visible && !TmpUtils.isUnicodeSpace(info.char)
    );
  }
  ...
  public static getColorExtraVertices(
    ...
    const result: [Color, Color, Color, Color] = [] as any;
    for (let i = 0; i < 4; i++) {
      const rColor: Color = dataList[offset + i]['colorExtra'].clone();
      result.push(rColor);
    }
    return result;
  }
  ...
  public static getPosVertices(
    ...
    const result: [Vec3, Vec3, Vec3, Vec3] = [] as any;
    for (let i = 0; i < 4; i++) {
      result.push(
        new Vec3(
          dataList[offset + i].x,
