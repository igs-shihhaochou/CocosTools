...
- [x] 建立編輯器擴展結構：
  ...
  └── uuid-resetter/
      ...
      └── README.md
  ```
- [x] 在 `package.json` 中註冊：
  - 右鍵資源管理器選單（透過 `assets-menu.js`）
  - 所有面板和主進程需要的 messages

### Task 2：UUID 掃描與映射建立
- [x] 遞迴掃描指定資料夾內所有 `.meta` 檔案
- [x] 收集所有現有 UUID（包含主資源 UUID 和 subMeta UUID）
- [x] 為每個舊 UUID 產生新的 UUID（使用 Cocos 相容的格式）
- [x] 建立 `oldUUID → newUUID` 的映射表
- [x] 處理 subMeta 的 UUID 格式（如 `uuid@6c48a`）：
  - 主 UUID 部分替換，hash 後綴保留
  - 確保 subMeta 的 key 對應關係正確

### Task 3：UUID 替換邏輯
- [x] 更新所有 `.meta` 檔案中的 UUID：
  - 頂層 `"uuid"` 欄位
  - `subMetas` 中每個子資源的 `"uuid"` 欄位
  - `userData` 中引用到的 UUID（如 `textureUuid`、`imageUuidOrDatabaseUri`）
- [x] 掃描並更新資料夾內所有引用到舊 UUID 的檔案：
...
- [x] 確保只替換映射表中有的 UUID（不動外部引用）

### Task 4：關聯完整性驗證
- [x] 替換完成後，重新掃描所有檔案
- [x] 驗證資料夾內部的所有 UUID 引用都指向有效的新 UUID
- [x] 列出「無法解析的引用」（可能指向資料夾外部的資源 — 這是正常的）
- [x] 產生替換報告：
  - 總共重置了多少個 UUID
  - 總共更新了多少個引用
  - 有無失敗或未能處理的項目
...
- [x] 執行時顯示進度：
  ...
  - 驗證進度
- [x] 完成後顯示結果報告
- [x] 提供「匯出映射表」功能（JSON 格式，方便除錯或回溯）

### Task 6：邊界情況處理
- [x] 處理 UUID 循環引用（A 引用 B，B 引用 A）
- [x] 處理同一 UUID 被多個檔案引用的情況
- [x] 忽略 `.meta` 以外的隱藏檔案
- [x] 不處理資料夾外部的引用（只更新選定資料夾內的檔案）
- [x] 支援巢狀 subMeta（如 SpriteFrame → Texture 的多層引用）
