    ...
    methods: {
        ...
        _startPolling() {
            this._pollTimer = setInterval(async () => {
                try {
                    const pending = await Editor.Message.request('missing-reference-checker', 'getPendingScan');
        ...
        async _fetchResult() {
            try {
                const result = await Editor.Message.request('missing-reference-checker', 'getLastResult');
                if (result) {
                    this._result = result;
                    this._renderResult();
                }
        ...
        async _doScanScene() {
            ...
            try {
                await Editor.Message.request('missing-reference-checker', 'scanCurrentScene');
                // 掃描完成後 fetchResult
                await this._fetchResult();
            }
            catch (e) {
        ...
        _renderResult() {
            ...
            tbody.querySelectorAll('.node-path-cell').forEach((cell) => {
                cell.addEventListener('click', () => {
                    ...
                    const uuid = (_a = row === null || row === void 0 ? void 0 : row.dataset) === null || _a === void 0 ? void 0 : _a.uuid;
                    if (uuid) {
