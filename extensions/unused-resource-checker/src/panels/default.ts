...
<div id="app" style="display:flex;flex-direction:column;height:100%;padding:12px;font-family:sans-serif;color:#ccc;">
    ...
    ready() {
        ...
        self._scanResult = null as ScanResult | null;
        self._checkedSet = new Set<string>();
        self._lastVersion = -1;

        self.$.btnSelectAll.addEventListener('click', () => {
            if (!self._scanResult) return;
        ...
        self.$.btnDelete.addEventListener('click', async () => {
            const toDelete = Array.from(self._checkedSet) as string[];
    ...
    methods: {
        ...
        async _doScan(folderUrl: string) {
            const self = this as any;
            ...
            try {
                const result: ScanResult | null = await Editor.Message.request(
                    'unused-resource-checker', 'scanForPanel', { scanFolder: folderUrl }
                );
        ...
        _getUniqueUnused(): UnusedAssetInfo[] {
            const self = this as any;
            if (!self._scanResult) return [];
            const seen = new Set<string>();
            const unique: UnusedAssetInfo[] = [];
            for (const a of self._scanResult.unusedAssets) {
                if (!seen.has(a.dbUrl)) {
                    seen.add(a.dbUrl);
                    unique.push(a);
